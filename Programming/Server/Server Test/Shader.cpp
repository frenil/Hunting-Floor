...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	// 6개의 텍스쳐를 사용한다.
	// 실제로 텍스쳐 배열 6에 연결되어 있다., 텍스쳐 2D 배열 _가 없는것이 Texture2DArray 형식의 타입, 0)
...
// 스카이 박스를
// 게임세상이 얼마의 크기를 가지고 있던지 상관 없이
// 어떤 크기(적당한 크기)를 가지는 육면체이다. (면의 방향이 안쪽을 향하는 면이다.)
// 그 육면체 내부에 카메라가 존재한다.
// 카메라가 이동을 하면 스카이박스를 카메라의 이동하는 양 만큼 똑같이 이동시킨다.
// 연속되는 이미지 6개를 생성한다.
// 텍스쳐 이미지의 거리감이 적당하면 육면체의 크기는 중요하지 않다.

// 후면버퍼를 지우는 것으로 사용했었다.
// 스카이박스를 제일 먼저 그리고 (이 상황에는 DepthTest를 안해도 된다.)
// DepthTest를 해서 그리되 Depth write는 하지 않아야 한다.	
//	(wirte 해버리면 스카이 박스 밖에 있는 애들은 하나도 안그려진다.)
// 그래서 Depth stencil desc을 해야한다.
D3D12_DEPTH_STENCIL_DESC CSkyBoxShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	d3dDepthStencilDesc.DepthEnable = FALSE;							// DephtTest 안함
	d3dDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;	// 무조건 write 하지 마
...
void CBillboardShader::BuildTrees(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, CCamera * p, void * pContext)
{
	// 지형 높이값 받기 위한 터레인
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	// 오브젝트 갯수에다가 셰이더 리소스뷰가 1
	CreateCbvAndSrvDescriptorHeaps(pd3dDevice, pd3dCommandList, m_nObjects, 1);
	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	CreateConstantBufferViews(pd3dDevice, pd3dCommandList, m_nObjects, m_pd3dcbGameObjects, ncbElementBytes);
	CreateShaderResourceViews(pd3dDevice, pd3dCommandList, pTexture, 7, false);
	...
	CTexturedRectMesh *pRectMesh = new CTexturedRectMesh(pd3dDevice, pd3dCommandList, 50.f, 50.f, 0.0f);

	m_ppObjects = new CGameObject*[m_nObjects];

	std::default_random_engine dre;
	std::uniform_real_distribution<> randPosX(1000.f, 1500.f);
	std::uniform_real_distribution<> randPosZ(500.f, 1000.f);
	dre.seed(std::chrono::system_clock::now().time_since_epoch().count());

	// 나무
	CBillboardObject *pBillboard = NULL;
	int objectCnt = 0;
	for (int i = 0; i < m_nObjects; ++i)
	{
		pBillboard = new CBillboardObject(1);
		pBillboard->SetMesh(0, pRectMesh);
		if (p != NULL) pBillboard->SetCamera(p);

#ifndef _WITH_BATCH_MATERIAL
		pBillboard->SetMaterial(pCubeMaterial);
#endif
		float xPosition = randPosX(dre);
		float zPosition = randPosZ(dre);
		float fHeight = pTerrain->GetHeight(xPosition, zPosition);
		pBillboard->SetPosition(xPosition, fHeight + 25.0f, zPosition);
		pBillboard->SetCbvGPUDescriptorHandlePtr(m_d3dCbvGPUDescriptorStartHandle.ptr + (::gnCbvSrvDescriptorIncrementSize * i));
		m_ppObjects[i] = pBillboard;
	}
}

void CBillboardShader::BuildGrasses(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, CCamera * p, void * pContext)
{
	// 지형 높이값 받기 위한 터레인
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	CTexturedRectMesh *pRectMesh = new CTexturedRectMesh(pd3dDevice, pd3dCommandList, 20.0f, 20.f, 0.0f);

	m_ppObjects = new CGameObject*[m_nObjects];

	std::default_random_engine dre;
	std::uniform_real_distribution<> randPosX(0.0f, 600);
	std::uniform_real_distribution<> randPosZ(0.0f, 600.f);
	dre.seed(std::chrono::system_clock::now().time_since_epoch().count());
	...
	for (int i = 0; i < m_nObjects; ++i)
	{
		...
		float zPosition = randPosZ(dre);
		float fHeight = pTerrain->GetHeight(xPosition, zPosition);
		pBillboard->SetPosition(xPosition, fHeight + 10.0f, zPosition);
		pBillboard->SetCbvGPUDescriptorHandlePtr(m_d3dCbvGPUDescriptorStartHandle.ptr + (::gnCbvSrvDescriptorIncrementSize * i));
		m_ppObjects[i] = pBillboard;
	}
}

void CBillboardShader::BuildFlowers(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, CCamera * p, void * pContext)
{
	// 지형 높이값 받기 위한 터레인
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	// 1개의 텍스쳐를 사용한다.
	CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/Trees/Tree09.dds", 0);
	...
	CTexturedRectMesh *pRectMesh = new CTexturedRectMesh(pd3dDevice, pd3dCommandList, 10, 10.f, 0.0f);
	...
	for (int i = 0; i < m_nObjects; ++i)
	{
		...
		float zPosition = randPosZ(dre);
		float fHeight = pTerrain->GetHeight(xPosition, zPosition);
		pBillboard->SetPosition(xPosition, fHeight + 5.f, zPosition);
		pBillboard->SetCbvGPUDescriptorHandlePtr(m_d3dCbvGPUDescriptorStartHandle.ptr + (::gnCbvSrvDescriptorIncrementSize * i));
		m_ppObjects[i] = pBillboard;
	}
}
...
void CBillboardShader::AnimateObjects(float fTimeElapsed, CCamera * p)
{
	for (int j = 0; j < m_nObjects; j++)
	{
		m_ppObjects[j]->Animate(fTimeElapsed, p);
	}
}
